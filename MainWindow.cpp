...
hash(const QByteArray &bytes)
{
	...
	const qint32 ByteMask = 0x3ff;

	qint32 result = 0;
	qint32 mask = ByteMask;

	for (int i = 0; i < bytes.count(); ++i) {

		// XXX
		//	The original code contains a bug!  Instead of
		//	'ByteMask' must be 'mask'.  Otherwise there is
		//	possibility, if bytes.count() > 1, that we return 16
		//	bits instead of just 10 (as stated in comment for
		//	ByteMask):
		//
		//	result |= (static_cast<quint8>(bytes[i]) & mask);
		//
		// Note
		//	We must reproduce that bug in order the tool work.

		result <<= 8;
		result |= (static_cast<quint8>(bytes[i]) & ByteMask);
		mask >>= 8;
		if (mask == 0)
			break;
	}
...
MainWindow::syncHashLineEdit()
{
	...
	md5.addData(passwordLineEdit()->text().toAscii());
	hashLineEdit()->setText(QString::number(hash(md5.result())));

	if (hashLineEdit()->text() == knownHashLineEdit()->text())
		QMessageBox::information(this, tr("Congradulation"),
			tr("Congradulation!\n\nThe hashes matches!"));
}
...
MainWindow::setupWidgets()
{
	...
	connect(
		knownSaltLineEdit(), SIGNAL(textChanged(const QString &)),
		this, SLOT(syncHashLineEdit()));

	Q_ASSERT(m_knownHashLineEdit == 0);
	m_knownHashLineEdit = new QLineEdit;
	connect(
		knownHashLineEdit(), SIGNAL(textChanged(const QString &)),
		this, SLOT(syncHashLineEdit()));

	Q_ASSERT(m_passwordLineEdit == 0);
	m_passwordLineEdit = new QLineEdit;
	connect(
		passwordLineEdit(), SIGNAL(textChanged(const QString &)),
		this, SLOT(syncHashLineEdit()));

	Q_ASSERT(m_hashLineEdit == 0);
...
http://src.chromium.org/svn/trunk/src/chrome/browser/sync/util/user_settings.cc
